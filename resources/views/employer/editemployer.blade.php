...
<div class="container">
  ...
  <!-- Modal -->
  <div class=" container col-lg-6 shadow rounded-3 p-2 bg-white" style="position: absolute; top:10%;left:16%">

      <div class="modal-content">
        <div class="">
          <h5 class="modal-title fw-bold" id="exampleModalLongTitle">Modifier un employer</h5>
        <hr>
        </div>
                @foreach ($employers as $employer )
                    
                    <form action="{{url('/employer/modifier')}}" method="POST">
                        @CSRF
                        ...
                        value="{{$employer->nom}}" required> <br>
                        ...
                        value="{{$employer->prenom}}" required> <br>
                        ...
                        value="{{$employer->poste}}" required> <br>
                
                        <label for="" class="m-1 fw-bold">Salaire</label>
                        <input type="text" name="salaire" id="" class="border-0 shadow-sm p-2 rounded-3 form-control-plaintext"
                        value="{{$employer->salaire}}" required> <br>
                        </div>
                        <hr>
                        <div class="modal-footer">
                        <button type="button" class="btn btn-sm rounded-3 btn-secondary" data-dismiss="modal">
                            <a href="{{url('/employer')}}" class="nav-link">annuler</a>
                        </button>
                        <button type="submit" class="btn btn-sm rounded-3 btn-primary">Modifier</button>
                        </div>
                    </form>
                @endforeach
      </div>
